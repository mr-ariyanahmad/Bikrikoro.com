...
import type { Category, Profile } from '@/types/product'
...
export default function AccountEdit() {
  ...
  useEffect(() => {
    const hydrate = (data: Profile | null) => {
      ...
      setBio(data?.bio ?? '')
      setPreferredCategoryId(data?.preferred_category_id ?? '')
      if (data?.preferred_category_id) window.localStorage.setItem('bikrikoro:profile-category:v1', data.preferred_category_id)
    ...
    supabase.from('categories').select('id,name,icon_url,sort_order').order('sort_order').then(({ data }) => setCategories((data ?? []) as Category[]))
  }, [cacheKey, uid])

  useEffect(() => {
    const normalized = username.trim().toLowerCase()
    if (!normalized || normalized === (profile?.username ?? '').toLowerCase()) { setUsernameStatus(normalized ? 'available' : 'idle'); return }
  ...
  }, [profile?.username, uid, username])

  const completionPercent = useMemo(() => {
    const isGoogle = user?.providerData.some((provider) => provider.providerId === 'google.com') ?? false
    const emailComplete = isGoogle || Boolean(user?.emailVerified)
    const completed = [Boolean(profile?.photo_url), Boolean(name.trim() && name.trim().length >= 2), Boolean(username.trim()), Boolean(phone.trim()), emailComplete, Boolean(bio.trim()), Boolean(preferredCategoryId), ...(isSeller ? [Boolean(shopName.trim()), Boolean(shopDescription.trim())] : [])].filter(Boolean).length
    const total = isSeller ? 9 : 7
    return Math.round((completed / total) * 100)
  }, [isSeller, name, username, phone, bio, preferredCategoryId, profile?.photo_url, shopDescription, shopName, user?.emailVerified, user?.providerData])
  ...
  const handleSave = async () => {
    ...
    if (username.trim() && usernameStatus !== 'available') { notify(usernameStatus === 'checking' ? 'Username availability check শেষ হওয়া পর্যন্ত অপেক্ষা করুন।' : 'এই username আগে থেকেই নেওয়া হয়েছে। অন্য username দিন।'); return }
    setSaving(true)
    const changes = { name: name.trim(), username: username.trim().toLowerCase() || null, phone: phone.trim() || null, gender: gender || null, date_of_birth: dateOfBirth || null, bio: bio.trim(), preferred_category_id: preferredCategoryId || null, ...(isSeller ? { shop_name: shopName.trim() || null, shop_description: shopDescription.trim() || null } : {}) }
    const { data, error } = await supabase.from('profiles').update(changes).eq('id', uid).select('*').maybeSingle()
    setSaving(false)
    if (error) { notify(error.code === '23505' ? 'এই username আগে থেকেই নেওয়া হয়েছে। অন্য username দিন।' : 'সেভ করা যায়নি — আবার চেষ্টা করুন।'); return }
  ...
  return <Layout wide><Helmet><title>প্রোফাইল সম্পাদনা | BikriKoro.Com</title></Helmet><div className="mx-auto w-full max-w-3xl pb-24"><div className="flex items-center gap-3"><button type="button" onClick={() => navigate('/account')} className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-outline bg-surface text-ink-700 shadow-sm transition hover:border-brand-500 hover:text-brand-700 active:scale-[0.97]" aria-label="প্রোফাইলে ফিরে যান"><ArrowLeft size={20} /></button><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-700">অ্যাকাউন্ট সেটিংস</p><h1 className="text-2xl font-bold text-ink-900">প্রোফাইল সম্পাদনা</h1></div></div>
    <section className="mt-5 overflow-hidden rounded-[1.35rem] border border-brand-100 bg-surface shadow-[0_14px_36px_rgba(15,23,42,0.065)]"><div className="bg-gradient-to-r from-brand-700 to-brand-500 px-5 py-4 text-white"><p className="text-xs text-white/80">আপনার পরিচয় ও শপের তথ্য</p><p className="mt-1 text-lg font-bold">{completionPercent}% সম্পূর্ণ</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/25"><div className="h-full rounded-full bg-white transition-all duration-500" style={{ width: `${completionPercent}%` }} /></div></div><div className="p-5 sm:p-6"><div className="flex flex-col items-center gap-3 border-b border-outline pb-6 sm:flex-row"><div className="relative flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-100 text-3xl font-bold text-brand-700">{profile?.photo_url ? <img src={profile.photo_url} alt="" className="h-full w-full object-cover" /> : displayName.charAt(0)}{uploadingPhoto && <span className="absolute inset-0 grid place-items-center bg-ink-900/45"><span className="h-6 w-6 animate-spin rounded-full border-2 border-white border-t-transparent" /></span>}</div><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-brand-200 bg-brand-50 px-4 py-2.5 text-sm font-bold text-brand-700 hover:border-brand-500"><Camera size={17} />{uploadingPhoto ? 'ছবি আপলোড হচ্ছে…' : 'ছবি পরিবর্তন করুন'}<input type="file" accept="image/*" className="hidden" disabled={uploadingPhoto} onChange={(event) => { const file = event.target.files?.[0]; if (file) void handlePhotoChange(file); event.target.value = '' }} /></label></div>
      <div className="mt-6 space-y-5"><label className="block"><span className="mb-1.5 block text-sm font-bold text-ink-900">আপনার নাম</span><input value={name} onChange={(event) => setName(event.target.value)} maxLength={80} className="w-full rounded-xl border border-outline bg-surface px-3 py-3 text-base outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10" placeholder="আপনার নাম লিখুন" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-sm font-bold text-ink-900">Username</span><input value={username} onChange={(event) => setUsername(event.target.value.replace(/[^a-zA-Z0-9_.-]/g, ''))} maxLength={30} className={`w-full rounded-xl border bg-surface px-3 py-3 text-base outline-none focus:border-brand-500 ${usernameStatus === 'taken' ? 'border-red-400' : usernameStatus === 'available' ? 'border-brand-400' : 'border-outline'}`} placeholder="your_username" />{usernameStatus === 'checking' && <span className="mt-1 block text-xs text-ink-500">Username check হচ্ছে…</span>}{usernameStatus === 'available' && <span className="mt-1 block text-xs font-semibold text-brand-700">✓ এই username নেওয়া যাবে</span>}{usernameStatus === 'taken' && <span className="mt-1 block text-xs font-semibold text-red-600">এই username আগে থেকেই নেওয়া হয়েছে</span>}</label><label className="block"><span className="mb-1.5 block text-sm font-bold text-ink-900">মোবাইল নম্বর</span><input value={phone} onChange={(event) => setPhone(event.target.value)} maxLength={20} className="w-full rounded-xl border border-outline bg-surface px-3 py-3 text-base outline-none focus:border-brand-500" placeholder="01XXXXXXXXX" /></label></div><div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-sm font-bold text-ink-900">Gender <span className="font-normal text-ink-400">(optional)</span></span><select value={gender ?? ''} onChange={(event) => setGender((event.target.value || null) as Profile['gender'])} className="w-full rounded-xl border border-outline bg-surface px-3 py-3 text-base outline-none focus:border-brand-500"><option value="">নির্বাচন করুন</option><option value="MALE">পুরুষ</option><option value="FEMALE">নারী</option><option value="OTHER">অন্যান্য</option><option value="PREFER_NOT_TO_SAY">বলতে চাই না</option></select></label><label className="block"><span className="mb-1.5 block text-sm font-bold text-ink-900">Date of Birth <span className="font-normal text-ink-400">(optional)</span></span><input type="date" value={dateOfBirth} onChange={(event) => setDateOfBirth(event.target.value)} className="w-full rounded-xl border border-outline bg-surface px-3 py-3 text-base outline-none focus:border-brand-500" /></label></div><label className="block"><span className="mb-1.5 block text-sm font-bold text-ink-900">আপনার পছন্দের Category</span><select value={preferredCategoryId} onChange={(event) => { setPreferredCategoryId(event.target.value); window.localStorage.setItem('bikrikoro:profile-category:v1', event.target.value) }} className="w-full rounded-xl border border-outline bg-surface px-3 py-3 text-base outline-none focus:border-brand-500"><option value="">সব category থেকে দেখান</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><span className="mt-1 block text-xs leading-5 text-ink-500">এই category-র product Home এবং recommendation-এ আগে দেখানো হবে।</span></label><label className="block"><span className="mb-1.5 block text-sm font-bold text-ink-900">ছোট Bio / About Me</span><textarea value={bio} onChange={(event) => setBio(event.target.value)} rows={4} maxLength={280} className="w-full resize-none rounded-xl border border-outline bg-surface px-3 py-3 text-base outline-none focus:border-brand-500" placeholder="নিজের সম্পর্কে সংক্ষেপে লিখুন" /><span className="mt-1 block text-right text-xs text-ink-400">{bio.length}/280</span></label>
