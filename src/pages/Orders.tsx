...
export default function Orders() {
  ...
  return (
    <Layout wide>
      ...
      {isSeller && <div className="mt-4 flex rounded-2xl border border-outline bg-surface p-1.5 shadow-[0_5px_14px_rgba(15,23,42,0.03)]"><button type="button" onClick={() => setTab('buying')} className={`flex-1 rounded-xl py-2.5 text-sm font-bold transition ${tab === 'buying' ? 'bg-brand-500 text-white shadow-sm' : 'text-ink-700 hover:bg-bg'}`}>কিনেছি</button><button type="button" onClick={() => setTab('selling')} className={`flex-1 rounded-xl py-2.5 text-sm font-bold transition ${tab === 'selling' ? 'bg-brand-500 text-white shadow-sm' : 'text-ink-700 hover:bg-bg'}`}>বিক্রি করেছি</button></div>}
      <div className="mt-4 flex flex-col gap-2 rounded-2xl border border-outline bg-surface p-3 shadow-[0_5px_14px_rgba(15,23,42,0.03)] sm:flex-row"><label className="relative flex-1"><Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink-400" /><input value={orderQuery} onChange={(event) => setOrderQuery(event.target.value)} placeholder="পণ্য বা অর্ডার নম্বর খুঁজুন..." className="w-full rounded-xl border border-outline bg-bg py-2.5 pl-9 pr-3 text-sm outline-none focus:border-brand-500" /></label><div className="min-w-52"><BrandSelect label="অর্ডারের অবস্থা" value={statusFilter} options={[{ value: 'ALL', label: 'সব অবস্থা' }, ...Object.entries(STATUS_LABEL).map(([value, label]) => ({ value, label }))]} onChange={(value) => setStatusFilter(value as OrderStatus | 'ALL')} /></div></div>

      {loadError && <p className="mt-4 rounded-xl border border-error/20 bg-error/5 p-3 text-sm font-medium text-error">অর্ডার লোড করা যায়নি: {loadError}</p>}
      ...
      {toast && <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-xl bg-ink-900 px-4 py-3 text-sm font-semibold text-white shadow-lg">{toast}</div>}
    </Layout>
  )
}
