...
export default function Sell() {
  ...
  const renderSpecField = (field: DigitalCategoryTemplate['fields'][number]) => {
    ...
    if (field.type === 'select') return <BrandSelect key={field.key} label={label} value={String(value)} options={(field.options ?? []).map((option) => ({ value: option, label: option }))} onChange={setValue} placeholder="বেছে নিন" />
  ...
  return (
    <Layout wide>
      <div className="mx-auto max-w-6xl">
        ...
        <nav aria-label="Listing progress" className="mt-7 rounded-xl border border-outline bg-surface p-4 sm:p-5">
        ...
        {reviewing ? <section className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="rounded-xl border border-outline bg-surface p-5 sm:p-7"><div className="flex items-start gap-3"><ClipboardCheck className="mt-0.5 text-brand-600" size={22} /><div><h2 className="text-xl font-bold text-ink-900">আপনার Listing প্রস্তুত</h2><p className="mt-1 text-sm text-ink-500">Publish করার আগে আপনার দেওয়া তথ্যগুলো একবার দেখে নিন।</p></div></div>
          ...
          {step === 1 && <section className="mt-6 rounded-xl border border-outline bg-surface p-5 sm:p-7"><div className="mb-6"><h2 className="text-xl font-bold text-ink-900">পণ্যের মূল তথ্য</h2><p className="mt-1 text-sm text-ink-500">আপনার ডিজিটাল পণ্য সম্পর্কে প্রাথমিক তথ্য দিন।</p></div><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
