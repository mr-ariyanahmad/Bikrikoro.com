...
export default function Sell() {
  ...
  const isValid =
    ...
    (!autoDeliveryEnabled || digitalDeliveryText.trim().length >= 3) &&
    images.length > 0 &&
  ...
  const validateStep = (targetStep: number) => {
    ...
    if (targetStep >= 3) {
      if (autoDeliveryEnabled && (!digitalDeliveryText.trim() || digitalDeliveryText.trim().length < 3)) return 'Automatic delivery চালু থাকলে secure delivery information দিন।'
  ...
  const renderSpecField = (field: DigitalCategoryTemplate['fields'][number]) => {
    ...
    if (field.type === 'textarea') return <label key={field.key} className="text-sm text-ink-700 sm:col-span-2"><span className="mb-1.5 block font-medium text-ink-900">{label}</span><textarea value={String(value)} onChange={(event) => setValue(event.target.value)} rows={3} className="w-full rounded-xl border border-outline bg-white px-3.5 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10" /></label>
    if (field.type === 'number') return <label key={field.key} className="text-sm text-ink-700"><span className="mb-1.5 block font-medium text-ink-900">{label}</span><input type="number" value={value === '' ? '' : String(value)} onChange={(event) => setValue(event.target.value ? Number(event.target.value) : '')} className="w-full rounded-xl border border-outline bg-white px-3.5 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10" /></label>
    ...
    return <label key={field.key} className="text-sm text-ink-700"><span className="mb-1.5 block font-medium text-ink-900">{label}</span><input type="text" value={String(value)} onChange={(event) => setValue(event.target.value)} className="w-full rounded-xl border border-outline bg-white px-3.5 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10" /></label>
  }
  ...
  return (
    <Layout wide fullWidth>
      <div id="listing-form" className="mx-auto w-full max-w-6xl scroll-mt-24 pb-10">
        <div className="relative overflow-hidden rounded-3xl border border-brand-100 bg-gradient-to-br from-brand-50 via-surface to-white p-5 shadow-sm sm:p-7">
          <div className="relative z-10"><span className="inline-flex items-center rounded-full border border-brand-200 bg-white/80 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-brand-700">Seller workspace</span><h1 className="mt-3 text-2xl font-bold tracking-tight text-ink-900 sm:text-3xl">{isEditing ? 'লিস্টিং এডিট করুন' : 'নতুন লিস্টিং তৈরি করুন'}</h1><p className="mt-1 text-sm text-ink-500">কম ধাপে আপনার digital product-এর একটি professional listing তৈরি করুন।</p></div>
          <div className="relative z-10 flex flex-wrap gap-2"><button type="button" onClick={handleSaveDraft} className="rounded-xl border border-brand-200 bg-white px-3.5 py-2.5 text-sm font-semibold text-ink-700 shadow-sm transition hover:border-brand-500 hover:text-brand-700">Save Draft</button>{!isEditing && <button type="button" onClick={handleClearDraft} className="rounded-lg border border-error/30 px-3 py-2 text-sm font-semibold text-error hover:bg-error/5">ড্রাফট মুছুন</button>}</div>
        </div>
        ...
        <nav aria-label="Listing progress" className="mt-5 rounded-2xl border border-outline bg-surface p-3 shadow-sm sm:p-5">
        ...
        {reviewing ? <section className="mt-4 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          {step === 2 && <section className="mt-5 overflow-hidden rounded-2xl border border-outline bg-surface shadow-sm"><div className="border-b border-outline bg-bg/60 px-5 py-5 sm:px-7"><h2 className="text-xl font-bold text-ink-900">পণ্যের বিস্তারিত তথ্য</h2><p className="mt-1 text-sm text-ink-500">আপনার নির্বাচিত category অনুযায়ী প্রয়োজনীয় তথ্য এখানে দেখানো হবে।</p></div><div className="px-5 py-5 sm:px-7 sm:py-7"><div className="mb-4"><h2 className="text-xl font-bold text-ink-900">পণ্যের বিস্তারিত তথ্য</h2><p className="mt-1 text-sm text-ink-500">আপনার নির্বাচিত category অনুযায়ী প্রয়োজনীয় তথ্য এখানে দেখানো হবে।</p></div>{selectedTemplate ? <><div className="border-l-2 border-brand-500 bg-brand-50/60 px-3 py-2"><p className="font-semibold text-ink-900">{selectedTemplate.name_bn}</p><p className="mt-1 text-sm text-ink-500">{selectedTemplate.description_bn || 'শুধু এই category-এর জন্য প্রাসঙ্গিক তথ্য পূরণ করুন।'}</p></div><div className="mt-4 grid gap-4 sm:grid-cols-2">{selectedTemplate.fields.filter((field) => field.key !== 'game_name').map(renderSpecField)}</div>{missingRequiredFields.length > 0 && <p className="mt-4 border-l-2 border-error bg-error/5 p-3 text-sm text-error">প্রয়োজনীয় তথ্য: {missingRequiredFields.map((field) => field.label_bn).join(', ')}</p>}</> : <p className="border-l-2 border-amber-500 bg-amber-50 p-3 text-sm text-amber-900">Step 1-এ একটি category নির্বাচন করুন।</p>}</div></section>}

          {step === 3 && <section className="mt-5 overflow-hidden rounded-2xl border border-outline bg-surface shadow-sm"><div className="border-b border-outline bg-bg/60 px-5 py-5 sm:px-7"><h2 className="text-xl font-bold text-ink-900">ডেলিভারি ও স্টক</h2><p className="mt-1 text-sm text-ink-500">ক্রেতা কীভাবে পণ্যটি পাবেন এবং আপনার পণ্যের availability নির্ধারণ করুন।</p></div><div className="px-5 py-5 sm:px-7 sm:py-7"><div className="mb-6"><h2 className="text-xl font-bold text-ink-900">ডেলিভারি ও স্টক</h2><p className="mt-1 text-sm text-ink-500">ক্রেতা কীভাবে পণ্যটি পাবেন এবং আপনার পণ্যের availability নির্ধারণ করুন।</p></div><div className="space-y-4"><div><h3 className="text-sm font-bold text-ink-900">Delivery Settings<HelpTip title="Delivery Settings">Automatic হলে আগে থেকে রাখা delivery content payment verified হওয়ার পরে system buyer-কে দিতে পারে। Manual হলে প্রতিটি paid order দেখে আপনি নিজে delivery দেবেন।</HelpTip></h3><div className="mt-3 grid gap-4 sm:grid-cols-2"><BrandSelect label="Delivery method *" value={autoDeliveryEnabled ? 'AUTOMATIC' : 'MANUAL'} options={[{ value: 'AUTOMATIC', label: 'Automatic' }, { value: 'MANUAL', label: 'Manual' }, { value: 'BOTH', label: 'Automatic + Manual' }]} onChange={(value) => setAutoDeliveryEnabled(value !== 'MANUAL')} /><BrandSelect label="Delivery time *" value={autoDeliveryEnabled ? '0' : fulfillmentWindowMinutes} options={[{ value: '0', label: 'Instant' }, { value: '5', label: 'Within 5 minutes' }, { value: '30', label: 'Within 30 minutes' }, { value: '60', label: 'Within 1 hour' }, { value: 'CUSTOM', label: 'Custom' }]} onChange={(value) => setFulfillmentWindowMinutes(value === 'CUSTOM' ? '' : value)} /></div>{autoDeliveryEnabled && <p className="mt-2 text-xs text-brand-700">অটোমেটিক ডেলিভারি হলে ০ মিনিট নির্বাচন করা হবে।</p>}</div><div className="border-t border-outline pt-6"><h3 className="text-sm font-bold text-ink-900">Stock<HelpTip title="Stock">Unlimited শুধু তখন ব্যবহার করুন যখন একই delivery বৈধভাবে একাধিক buyer-কে দেওয়া যায়। আলাদা key বা account থাকলে quantity বা key pool ব্যবহার করুন।</HelpTip></h3><div className="mt-3 grid gap-4 sm:grid-cols-2"><BrandSelect label="Stock type" value={stockMode} options={[{ value: 'QUANTITY', label: 'Limited stock' }, { value: 'UNLIMITED', label: 'Unlimited' }, { value: 'KEY_POOL', label: 'Available on request / key pool' }]} onChange={(value) => setStockMode(value as 'UNLIMITED' | 'QUANTITY' | 'KEY_POOL')} />{stockMode === 'QUANTITY' && <label className="text-sm font-semibold text-ink-900">Stock quantity<input type="number" min="1" value={stockQuantity} onChange={(event) => setStockQuantity(event.target.value)} placeholder="10" className="mt-1.5 w-full rounded-xl border border-outline bg-white px-3.5 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10" /></label>}</div><button type="button" aria-pressed={deactivateWhenOutOfStock} onClick={() => setDeactivateWhenOutOfStock((value) => !value)} className={`mt-4 w-full rounded-lg border px-3 py-3 text-left text-sm ${deactivateWhenOutOfStock ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-outline text-ink-600'}`}>স্টক শেষ হলে পণ্যটি সক্রিয় রাখুন <span className="float-right font-bold">{deactivateWhenOutOfStock ? 'চালু' : 'বন্ধ'}</span></button><p className="mt-1 text-xs text-ink-500">চালু থাকলে স্টক ০ হলেও product listing ক্রেতাদের কাছে দেখা যাবে।</p></div><div className="border-t border-outline pt-6"><h3 className="text-sm font-bold text-ink-900">Product availability</h3><div className="mt-3 grid gap-4 sm:grid-cols-2"><BrandSelect label="Region / Country" value={regionCode} options={FIELD_OPTIONS.region_code.map((option) => ({ value: option, label: option }))} onChange={setRegionCode} placeholder="GLOBAL" /><BrandSelect label="Validity / Subscription" value={subscriptionPeriod} options={FIELD_OPTIONS.subscription_period.map((option) => ({ value: option, label: option }))} onChange={setSubscriptionPeriod} placeholder="মেয়াদ বেছে নিন" /><BrandSelect label="Warranty / Replacement" value={warrantyPeriod} options={FIELD_OPTIONS.warranty_period.map((option) => ({ value: option, label: option }))} onChange={setWarrantyPeriod} placeholder="Warranty বেছে নিন" /></div></div><div className="border-t border-outline pt-5">{autoDeliveryEnabled ? <><h3 className="text-sm font-bold text-ink-900">{deliveryCopy.label}<HelpTip title={deliveryCopy.label}>Automatic delivery চালু থাকলে payment verified হওয়ার পর buyer এই protected information পাবেন। এখানে শুধু সেই product-এর প্রয়োজনীয় key, account access বা download instruction দিন।</HelpTip></h3><p className="mt-1 text-sm text-ink-500">অর্ডার সম্পন্ন হওয়ার পর ক্রেতা যে তথ্য পাবেন তা এখানে দিন।</p><textarea value={digitalDeliveryText} onChange={(event) => setDigitalDeliveryText(event.target.value)} rows={6} placeholder={deliveryCopy.placeholder} className="mt-3 w-full rounded-xl border border-outline bg-white px-3.5 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10" /><div className="mt-2 rounded-lg border border-brand-100 bg-brand-50 px-3 py-2.5 text-xs leading-5 text-brand-800">এই তথ্য payment verified হওয়ার পর সংশ্লিষ্ট ক্রেতাকে দেখানো হবে। BikriKoro staff সাধারণ workflow-এ এই content দেখার জন্য নয়; নিরাপত্তা ও access policy সম্পর্কে বিস্তারিত <a href="/seller-privacy-policy" className="font-semibold underline">Seller Privacy Policy</a>-তে পড়ুন।</div></> : <div className="border-l-2 border-sky-500 bg-sky-50/70 px-3 py-2.5"><h3 className="text-sm font-bold text-ink-900">Manual delivery information <HelpTip title="Manual delivery information">Manual delivery বেছে নিলে এখানে কোনো password, key বা secret দেওয়ার প্রয়োজন নেই। Payment verified হওয়ার পরে Seller Dashboard/Orders থেকে প্রতিটি order অনুযায়ী buyer-কে delivery দেবেন।</HelpTip></h3><p className="mt-1 text-sm text-ink-600">এই listing-এর জন্য secure secret এখন প্রয়োজন নেই। Payment verified হলে order থেকে buyer-কে প্রয়োজনীয় information manually পাঠাবেন।</p></div>}</div><div><label className="text-sm font-semibold text-ink-900">ক্রেতার জন্য ডেলিভারি নির্দেশনা <span className="font-normal text-ink-400">— Optional</span><textarea value={deliveryNote} onChange={(event) => setDeliveryNote(event.target.value)} rows={4} placeholder="পেমেন্টের পরে কীভাবে পণ্যটি ব্যবহার, activate বা redeem করতে হবে তা লিখুন।" className="mt-1.5 w-full rounded-xl border border-outline bg-white px-3.5 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10" /></label></div>{stockMode === 'KEY_POOL' && <div className="rounded-lg border border-outline bg-bg p-4"><p className="text-sm font-semibold text-ink-900">Key inventory: {availableKeyCount}</p>{isEditing ? <><textarea value={keyBatchText} onChange={(event) => setKeyBatchText(event.target.value)} rows={3} placeholder="প্রতি লাইনে একটি key" className="mt-3 w-full rounded-xl border border-outline bg-white px-3.5 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10" /><button type="button" onClick={() => void handleAddLicenseKeys()} disabled={!keyBatchText.trim()} className="mt-2 rounded-lg border border-brand-500 px-3 py-2 text-sm font-semibold text-brand-700 disabled:opacity-50">Key যোগ করুন</button></> : <p className="mt-1 text-xs text-ink-500">লিস্টিং publish করার পর key inventory যোগ করা যাবে।</p>}</div>}</div></div></section>}

          {error && <p className="mt-5 rounded-xl border border-error/30 bg-error/5 p-4 text-sm leading-6 text-error">{error}</p>}
          <div className="sticky bottom-3 z-20 mt-5 flex flex-col-reverse gap-2 rounded-2xl border border-outline bg-white/95 p-3 shadow-lg backdrop-blur sm:static sm:flex-row sm:justify-between sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none"><button type="button" onClick={() => step === 1 ? navigate('/my-listings') : goToStep(step - 1)} className="inline-flex items-center justify-center gap-2 rounded-lg border border-outline px-4 py-3 text-sm font-semibold text-ink-700"><ChevronLeft size={17} />{step === 1 ? 'Cancel' : 'Back'}</button><div className="flex flex-col gap-2 sm:flex-row"><button type="button" onClick={handleSaveDraft} className="rounded-lg border border-outline px-4 py-3 text-sm font-semibold text-ink-700">Save Draft</button>{step < 3 ? <button type="button" onClick={() => goToStep(step + 1)} className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-600 px-5 py-3 text-sm font-bold text-white">Continue <ChevronRight size={17} /></button> : <button type="button" onClick={() => { const validation = validateStep(3); if (validation) setError(validation); else { setError(null); setReviewing(true) } }} className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-600 px-5 py-3 text-sm font-bold text-white">Review Listing <ClipboardCheck size={17} /></button>}</div></div>
        </>}
      </div>
    </Layout>
  )
