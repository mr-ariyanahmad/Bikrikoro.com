...
export default function OrderDetail() {
  ...
  return (
    <Layout>
      ...
      <InvoiceReceipt order={order} orderNumber={readableOrderNumber} />
    </Layout>
  )
}

function InvoiceReceipt({ order, orderNumber }: { order: Order; orderNumber: string }) {
  return <div className="invoice-print-only" aria-hidden="true"><div className="invoice-paper"><div className="invoice-header"><div><div className="invoice-brand-mark"><img src="/brand/icon-source.jpg" alt="BikriKoro logo" /></div><p className="invoice-brand-name">BikriKoro<span>.Com</span></p><p className="invoice-muted">নিরাপদ ডিজিটাল মার্কেটপ্লেস</p></div><div className="invoice-title"><p>অর্ডার রসিদ</p><span>INVOICE</span></div></div><div className="invoice-meta"><div><span>অর্ডার নম্বর</span><strong>{orderNumber}</strong></div><div><span>তারিখ</span><strong>{formatDate(order.created_at)}</strong></div><div><span>স্ট্যাটাস</span><strong>{STATUS_LABEL[order.status]}</strong></div></div><div className="invoice-section-title">পণ্যের বিবরণ</div><div className="invoice-product"><div className="invoice-product-image">{order.product_image && <img src={order.product_image} alt="" />}</div><div><strong>{order.product_title}</strong><span>পরিমাণ: {order.quantity}</span><span>ডিজিটাল পণ্য</span></div></div><div className="invoice-total"><div><span>মূল দাম</span><strong>{formatTaka(order.subtotal ?? order.price)}</strong></div>{Boolean(order.discount_amount) && <div><span>কুপন ছাড় {order.coupon_code ? `(${order.coupon_code})` : ''}</span><strong>-{formatTaka(order.discount_amount ?? 0)}</strong></div>}{Boolean(order.discount_amount) && <div><span>ছাড়ের উৎস</span><strong>{order.coupon_funding_source === 'SELLER' ? 'সেলার' : 'BikriKoro platform'}</strong></div>}<div><span>পণ্যের দাম</span><strong>{formatTaka(order.price)}</strong></div><div><span>এসক্রো ফি</span><strong>{formatTaka(order.escrow_fee)}</strong></div><div className="invoice-grand-total"><span>সর্বমোট</span><strong>{formatTaka(order.price + order.escrow_fee)}</strong></div></div>{order.delivery_email && <div className="invoice-note"><strong>ডেলিভারি ইমেইল</strong><span>{order.delivery_email}</span></div>}<div className="invoice-secure"><span>✓</span><div><strong>নিরাপদ ডিজিটাল লেনদেন</strong><p>পেমেন্ট ও ডেলিভারি BikriKoro escrow সুরক্ষার মাধ্যমে পরিচালিত।</p></div></div><div className="invoice-footer"><span>ধন্যবাদ — BikriKoro.Com</span><span>support@bikrikoro.com</span></div></div></div>
}
