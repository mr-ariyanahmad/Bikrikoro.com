...
import { ArrowLeft, BadgeCheck, ChevronRight, CircleAlert, CreditCard, FileText, Headphones, LockKeyhole, MessageCircle, Package, Paperclip, Plus, Send, ShieldCheck } from 'lucide-react'
...
type SupportMessage = { id: string; case_id: string; sender_id: string; sender_role: 'CUSTOMER' | 'SUPPORT'; text: string; attachments: SupportAttachment[]; created_at: string; read_at?: string | null }
...
export default function SupportCenter() {
  ...
  const sendMessage = async () => {
    if (!selectedCase || (!message.trim() && pendingFiles.length === 0) || saving || ['RESOLVED', 'CLOSED'].includes(selectedCase.status)) return
    const text = message.trim(); setSaving(true); setError(null)
    try {
      const attachments = pendingFiles.length ? await uploadSupportAttachments(pendingFiles, user?.uid ?? 'customer') : []
      await chatRequest({ action: 'support_send_message', caseId: selectedCase.id, text, attachments })
  ...
  return <Layout wide fullScreen hideFooter hideMobileQuickNav backFallback="/chat" backLabel="চ্যাটে ফিরুন">
    <div className={hasCases ? 'mx-auto flex h-full w-full max-w-4xl flex-col overflow-hidden' : 'mx-auto h-full w-full max-w-3xl overflow-y-auto pb-8'}>
      ...
      {loading ? <div className="mt-5 h-32 animate-pulse rounded-2xl bg-outline/40" /> : hasCases && <div className="mt-4 flex min-h-0 flex-1 flex-col overflow-hidden border-y border-outline bg-surface"><div className="shrink-0 border-b border-outline bg-surface p-3"><div className="flex gap-2 overflow-x-auto">{cases.map((item) => { const category = categoryMap.get(item.category); return <button type="button" key={item.id} onClick={() => setSelectedCase(item)} className={`min-w-[9.5rem] shrink-0 rounded-xl border px-3 py-2 text-left transition ${selectedCase?.id === item.id ? 'border-brand-400 bg-brand-50' : 'border-outline bg-bg hover:border-brand-200'}`}><p className="text-[10px] font-bold text-brand-700">CASE #BK-{item.case_number}</p><p className="mt-1 truncate text-xs font-bold text-ink-900">{category?.title ?? item.subject}</p><span className={`mt-1 inline-block rounded-full px-1.5 py-0.5 text-[9px] font-bold ${item.status === 'RESOLVED' || item.status === 'CLOSED' ? 'bg-ink-100 text-ink-500' : 'bg-brand-100 text-brand-700'}`}>{statusLabels[item.status]}</span></button> })}</div></div><div className="flex min-h-0 flex-1 flex-col"><div className="flex shrink-0 items-center gap-2 border-b border-outline bg-bg px-3 py-2.5"><span className="grid h-8 w-8 place-items-center rounded-lg bg-brand-50 text-brand-600"><SelectedIcon size={16} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-ink-900">{selectedCategory?.title ?? 'Support Case'}</p><p className="text-[10px] text-ink-500">CASE #BK-{selectedCase?.case_number} · {selectedCase ? statusLabels[selectedCase.status] : ''}</p></div></div><div className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-bg p-3">{messagesLoading ? <div className="h-20 animate-pulse rounded-xl bg-outline/40" /> : messages.length === 0 ? <div className="py-12 text-center text-sm text-ink-500"><MessageCircle size={32} className="mx-auto mb-3 text-ink-300" />এই Case-এ এখনো কোনো message নেই।</div> : messages.map((item) => <div key={item.id} className={`flex ${item.sender_role === 'CUSTOMER' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] rounded-2xl px-3.5 py-2.5 text-sm ${item.sender_role === 'CUSTOMER' ? 'rounded-br-md bg-brand-500 text-white' : 'rounded-bl-md bg-surface text-ink-900 shadow-sm'}`}><p className="whitespace-pre-wrap break-words">{item.text}</p>{item.attachments?.length > 0 && <div className="mt-2 space-y-1.5">{item.attachments.map((attachment) => attachment.type.startsWith('image/') ? <a key={attachment.url} href={attachment.url} target="_blank" rel="noreferrer"><img src={attachment.url} alt={attachment.name} className="max-h-44 max-w-full rounded-lg object-cover" /></a> : <a key={attachment.url} href={attachment.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-lg bg-black/10 px-2.5 py-2 text-xs underline"><FileText size={15} />{attachment.name}</a>)}</div>}<p className={`mt-1 text-[10px] ${item.sender_role === 'CUSTOMER' ? 'text-white/70' : 'text-ink-400'}`}>{formatDateTime(item.created_at)}{item.sender_role === 'CUSTOMER' && item.read_at ? ' · Seen' : ''}</p></div></div>)}</div><div className="shrink-0 border-t border-outline bg-surface p-3">{pendingFiles.length > 0 && <div className="mb-2 flex gap-1.5 overflow-x-auto">{pendingFiles.map((file) => <span key={`${file.name}-${file.size}`} className="shrink-0 rounded-lg bg-brand-50 px-2 py-1 text-[10px] text-brand-700">{file.name}</span>)}</div>}<div className="flex items-center gap-2"><label className="grid h-10 w-10 shrink-0 cursor-pointer place-items-center rounded-full text-ink-500 hover:bg-brand-50 hover:text-brand-600" aria-label="ফাইল বা ছবি attach"><Paperclip size={19} /><input type="file" multiple accept="image/*,.pdf,.txt,.doc,.docx,.zip" className="hidden" onChange={(event) => setPendingFiles(Array.from(event.target.files ?? []).slice(0, 5))} /></label><input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && void sendMessage()} disabled={!selectedCase || selectedCase.status === 'RESOLVED' || selectedCase.status === 'CLOSED'} placeholder={selectedCase?.status === 'RESOLVED' || selectedCase?.status === 'CLOSED' ? 'এই Case সমাধান হয়েছে' : 'মেসেজ লিখুন...'} className="min-w-0 flex-1 rounded-full border border-outline bg-bg px-4 py-2.5 text-sm outline-none focus:border-brand-500 disabled:opacity-60" /><button type="button" onClick={() => void sendMessage()} disabled={(!message.trim() && pendingFiles.length === 0) || saving || !selectedCase} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-brand-500 text-white disabled:opacity-50" aria-label="মেসেজ পাঠান"><Send size={16} /></button></div></div></div></div>}
