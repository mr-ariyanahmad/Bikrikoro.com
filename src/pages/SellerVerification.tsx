...
import { ArrowRight, CheckCircle2, Eye, FileCheck2, ShieldCheck, Upload, X } from 'lucide-react'
...
export default function SellerVerification() {
  ...
  const isValid = !statusLoadError && Boolean(businessType) && fullName.trim().length >= 3 && phone.replace(/\D/g, '').length >= 11 && idNumber.trim().length >= 5 && address.trim().length >= 5 && (businessType === 'PERSONAL' || businessName.trim().length >= 2) && requiredRequirements.every((item) => files[item.document_type])
