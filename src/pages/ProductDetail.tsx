...
export default function ProductDetail() {
  ...
  const isDemoListing = product.title.startsWith('TEST / Demo only —') || digitalSpecs?.specifications?.test_data === true
  ...
  return (
    <Layout wide hideFooter={showBuy} hideMobileHeader mobileBottomBar>
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            offers: {
              ...
              price: product.price,
              priceCurrency: 'BDT',
              availability: isDemoListing ? 'https://schema.org/OutOfStock' : 'https://schema.org/InStock',
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-8">
        ...
        <div>
          ...
          <div className="mt-1.5">
            <p className={`whitespace-pre-line text-[15px] leading-7 text-ink-600 [text-align:justify] ${descriptionExpanded ? '' : 'line-clamp-5'}`}>
          ...
          <div className="mt-4 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-xs leading-relaxed text-amber-950 shadow-sm">
            <p className="flex items-center gap-2 font-bold text-amber-900"><ShieldCheck size={15} className="text-amber-600" />নিরাপদে কেনাকাটার টিপস</p>
          ...
          <div className="mt-6 hidden space-y-2 md:block">
            {isOwnListing ? (
              ...
              <div className="w-full rounded-xl border border-amber-300 bg-amber-50 py-3 text-center text-sm font-semibold text-amber-900">
                TEST / Demo only — অর্ডার বন্ধ
              </div>
            ) : user ? (
              <>
                <button
