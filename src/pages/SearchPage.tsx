...
import { loadPopularSearches, loadSearchProductScores, recordSearchEvent } from '@/lib/searchAnalytics'
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    void loadPopularSearches().then(setTrending)
    void (async () => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const [{ data, error }, foundShops, searchScores] = await Promise.all([request.limit(query.length >= 2 ? 40 : 12), searchPublicShops(query), loadSearchProductScores(query)])
      if (!active) return
      ...
      const nextProducts = rankSearchProductsByInterest(uniqueProducts, getRecentlyViewedIds())
        .filter((product) => !isTestDemoProduct(product))
        .sort((left, right) => (searchScores.get(right.id) ?? 0) - (searchScores.get(left.id) ?? 0))
      setProducts(nextProducts)
      setShops(foundShops)
      if (query.length >= 2) recordSearchEvent(query)
  ...
  const goToResults = (term: string) => { recordSearchEvent(term); navigate(`/products?q=${encodeURIComponent(term)}`) }
  ...
  return <Layout fullScreen hideFooter hideMobileQuickNav backFallback="/" backLabel="হোমে ফিরুন">
    <div className="-mx-4 flex min-h-0 flex-1 flex-col bg-bg sm:mx-0">
      <div className="flex-1 overflow-y-auto px-4 pb-7 pt-4 sm:mx-auto sm:w-full sm:max-w-5xl sm:px-0">
        <section className="mb-5 rounded-3xl border border-brand-100 bg-gradient-to-br from-brand-50 via-white to-emerald-50/50 p-4 shadow-[0_8px_24px_rgba(1,124,80,0.07)] sm:p-5"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-700">BikriKoro search</p><h1 className="mt-1 text-xl font-extrabold tracking-tight text-ink-900 sm:text-2xl">যা খুঁজছেন, সহজেই খুঁজে নিন</h1></div><span className="hidden rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-ink-500 shadow-sm sm:inline-flex">⌘ K / Ctrl K</span></div><SearchBar compact /></section>
        {query.length < 2 ? <>
          ...
          {trending.length > 0 && <section className="border-b border-outline py-5"><h2 className="flex items-center gap-2 text-lg font-bold text-ink-900"><Flame size={19} className="text-amber-500" />জনপ্রিয় সার্চ</h2><p className="mt-1 text-xs text-ink-400">গত ৩০ দিনে মানুষ যে শব্দগুলো বেশি খুঁজেছে</p><div className="mt-3 flex flex-wrap gap-2">{trending.map((term) => <button key={term} type="button" onClick={() => goToResults(term)} className="rounded-full bg-brand-50 px-3.5 py-2 text-sm font-semibold text-brand-700 active:scale-[0.98]">{term}</button>)}</div></section>}
