...
export default function Home() {
  ...
  return <Layout wide>
    ...
    <div className="md:hidden">
      <section className="-mx-4 -mt-7 overflow-hidden bg-gradient-to-b from-brand-50 via-amber-50/60 to-bg px-4 pb-4 pt-3">
        <div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-[11px] font-extrabold uppercase tracking-[0.08em] text-brand-700">BikriKoro marketplace</p><h1 className="mt-1 text-[1.35rem] font-extrabold leading-tight tracking-tight text-ink-900">{isSeller ? 'আজ কী কিনবেন বা বিক্রি করবেন?' : 'আজ কী কিনবেন?'}</h1></div><Link to={isSeller ? '/sell' : '/become-seller'} aria-label={isSeller ? 'পণ্য পোস্ট করুন' : 'সেলার হোন'} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-brand-600 text-white shadow-[0_8px_18px_rgba(1,124,80,0.25)]">{isSeller ? <Plus size={22} /> : <Store size={20} />}</Link></div>
        <div className="mt-3 grid grid-cols-3 gap-2 rounded-2xl border border-white/90 bg-white/85 p-2 shadow-[0_12px_30px_rgba(15,23,42,0.08)]"><Link to="/wallet" className="min-w-0 rounded-2xl bg-brand-50 px-3 py-3"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white text-brand-700 shadow-sm"><WalletCards size={17} /></span><p className="mt-2 truncate text-[10px] font-semibold text-brand-700">ওয়ালেট</p><p className="mt-0.5 truncate tabular-amount text-sm font-bold text-ink-900">{user ? formatTaka(balance) : 'লগইন'}</p></Link><button type="button" onClick={() => void checkIn()} disabled={checkInLoading || checkedIn} className="min-w-0 rounded-2xl bg-amber-50 px-3 py-3 text-left disabled:opacity-60"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white text-amber-600 shadow-sm">{checkInLoading ? <Loader2 size={17} className="animate-spin" /> : <Coins size={17} />}</span><p className="mt-2 truncate text-[10px] font-semibold text-amber-700">দৈনিক চেক-ইন</p><p className="mt-0.5 truncate text-sm font-bold text-ink-900">{checkedIn ? 'নেওয়া হয়েছে' : '+১০ কয়েন'}</p></button><Link to="/products" className="min-w-0 rounded-2xl bg-sky-50 px-3 py-3"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white text-sky-700 shadow-sm"><ShieldCheck size={17} /></span><p className="mt-2 truncate text-[10px] font-semibold text-sky-700">ডিজিটাল সুরক্ষা</p><p className="mt-0.5 truncate text-sm font-bold text-ink-900">এসক্রো নিরাপদ</p></Link></div>
      </section>
      {checkInMessage && <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">{checkInMessage}</p>}
      <div className="mt-5 overflow-hidden rounded-2xl border border-outline/80 bg-surface p-2.5 shadow-[0_6px_16px_rgba(15,23,42,0.035)]"><CategoryPills categories={categories} selectedId={null} onSelect={handleCategorySelect} /></div>
      <section className="mt-5"><div className="flex items-center justify-between px-0.5"><div><p className="text-[11px] font-bold uppercase tracking-[0.08em] text-brand-700">বেছে নিন</p><h2 className="mt-0.5 text-lg font-extrabold text-ink-900">জনপ্রিয় পণ্য</h2></div><Link to="/products" className="text-xs font-bold text-brand-700">সব দেখুন →</Link></div>{productGrid}</section>
