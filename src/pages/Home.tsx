  ...
  return <Layout wide>
    ...
    <div className="md:hidden">
      <section className="-mx-4 -mt-7 overflow-hidden bg-gradient-to-b from-brand-50 via-amber-50/60 to-bg px-4 pb-5 pt-5">
        <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-brand-700">BikriKoro digital marketplace</p><h1 className="mt-1 text-xl font-bold tracking-tight text-ink-900">{isSeller ? 'আজ কী কিনবেন বা বিক্রি করবেন?' : 'আজ কী কিনবেন?'}</h1></div><Link to={isSeller ? '/sell' : '/become-seller'} aria-label={isSeller ? 'পণ্য পোস্ট করুন' : 'সেলার হোন'} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-brand-600 text-white shadow-[0_8px_18px_rgba(1,124,80,0.25)]">{isSeller ? <Plus size={22} /> : <Store size={20} />}</Link></div>
