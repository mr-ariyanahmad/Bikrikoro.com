...
import { ArrowRight, BookOpen, Check, Coins, Heart, Loader2, Package, Plus, ShieldCheck, ShoppingBag, Sparkles, WalletCards, Zap } from 'lucide-react'
  ...
  return <Layout wide>
    ...
    <div className="md:hidden">
      <section className="-mx-4 -mt-7 overflow-hidden bg-gradient-to-b from-brand-50 via-amber-50/60 to-bg px-4 pb-5 pt-5">
        ...
        <div className="mt-4 grid grid-cols-3 overflow-hidden rounded-2xl border border-white/80 bg-white/90 shadow-[0_10px_26px_rgba(15,23,42,0.08)]"><Link to="/wallet" className="min-w-0 border-r border-outline/70 px-3 py-3"><WalletCards size={19} className="text-brand-600" /><p className="mt-2 truncate text-[10px] font-semibold text-ink-500">ওয়ালেট</p><p className="mt-0.5 truncate tabular-amount text-sm font-bold text-ink-900">{user ? formatTaka(balance) : 'লগইন'}</p></Link><button type="button" onClick={() => void checkIn()} disabled={checkInLoading || checkedIn} className="min-w-0 border-r border-outline/70 px-3 py-3 text-left disabled:opacity-60"><Coins size={19} className="text-amber-500" /><p className="mt-2 truncate text-[10px] font-semibold text-ink-500">দৈনিক চেক-ইন</p><p className="mt-0.5 truncate text-sm font-bold text-ink-900">{checkedIn ? 'নেওয়া হয়েছে' : '+১০ কয়েন'}</p></button><Link to="/products" className="min-w-0 px-3 py-3"><ShieldCheck size={19} className="text-brand-600" /><p className="mt-2 truncate text-[10px] font-semibold text-ink-500">ডিজিটাল সুরক্ষা</p><p className="mt-0.5 truncate text-sm font-bold text-brand-700">এসক্রো নিরাপদ</p></Link></div>
      </section>
      {checkInMessage && <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">{checkInMessage}</p>}
      <section className="mt-5 grid grid-cols-5 gap-2"><Link to="/products" className="flex min-w-0 flex-col items-center gap-1.5 text-center"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-50 text-brand-700 shadow-sm"><ShoppingBag size={20} /></span><span className="text-[10px] font-semibold text-ink-700">সব পণ্য</span></Link><Link to="/orders" className="flex min-w-0 flex-col items-center gap-1.5 text-center"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-sky-50 text-sky-700 shadow-sm"><Package size={20} /></span><span className="text-[10px] font-semibold text-ink-700">অর্ডার</span></Link><Link to="/library" className="flex min-w-0 flex-col items-center gap-1.5 text-center"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-violet-50 text-violet-700 shadow-sm"><BookOpen size={20} /></span><span className="text-[10px] font-semibold text-ink-700">লাইব্রেরি</span></Link><Link to="/favorites" className="flex min-w-0 flex-col items-center gap-1.5 text-center"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-rose-50 text-rose-600 shadow-sm"><Heart size={20} /></span><span className="text-[10px] font-semibold text-ink-700">পছন্দের</span></Link><Link to="/sell" className="flex min-w-0 flex-col items-center gap-1.5 text-center"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-50 text-amber-700 shadow-sm"><Sparkles size={20} /></span><span className="text-[10px] font-semibold text-ink-700">বিক্রি</span></Link></section>
      <section className="mt-6"><div className="flex items-center justify-between"><h2 className="text-sm font-bold text-ink-900">আজকের ডিজিটাল ডিল</h2><Link to="/products" className="inline-flex items-center gap-0.5 text-xs font-bold text-brand-700">সব দেখুন <ArrowRight size={14} /></Link></div><div className="scrollbar-none mt-3 flex gap-3 overflow-x-auto pb-1">{banners.length > 0 ? banners.map((banner) => <Link key={banner.id} to={banner.target_category_id ? `/products?category=${banner.target_category_id}` : '/products'} className="h-32 w-56 shrink-0 overflow-hidden rounded-2xl bg-outline/30 shadow-sm"><img src={banner.image_url} alt="" className="h-full w-full object-cover" /></Link>) : <><Link to="/products" className="w-56 shrink-0 rounded-2xl bg-gradient-to-br from-brand-600 to-brand-800 p-4 text-white shadow-sm"><Zap size={19} className="text-amber-300" /><p className="mt-4 text-base font-bold">অটো ডেলিভারি ডিল</p><p className="mt-1 text-xs text-brand-50">পেমেন্ট নিশ্চিত হলেই দ্রুত অর্ডার প্রস্তুত</p></Link><Link to="/sell" className="w-56 shrink-0 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 p-4 text-ink-900 shadow-sm"><Sparkles size={19} /><p className="mt-4 text-base font-bold">আপনার পণ্য বিক্রি করুন</p><p className="mt-1 text-xs font-medium text-ink-800/80">নিরাপদ ডিজিটাল বিক্রির জন্য পোস্ট দিন</p></Link></>}</div></section>
