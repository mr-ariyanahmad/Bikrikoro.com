...
export default function AdminPaymentAccounts() {
  ...
  return <AdminTableCard className="mt-5"><div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-brand-50 text-brand-700"><WalletCards size={19} /></span><div><h2 className="font-semibold text-slate-900">Seller payout ও buyer refund account</h2><p className="mt-1 text-xs leading-5 text-slate-500">Admin এখান থেকে account attach, edit, default এবং remove করতে পারবেন। Account number কেবল admin workspace-এ দেখানো হয়।</p></div></div><div className="flex gap-2"><button type="button" onClick={() => { setForm(emptyForm()); setFormOpen(true); setError(null) }} className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-brand-500 px-3 py-2 text-xs font-bold text-white"><Plus size={15} />Account attach</button><button type="button" onClick={() => void load()} className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600"><RefreshCw size={14} />Refresh</button></div></div>
