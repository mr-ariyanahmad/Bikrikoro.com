...
export default function AdminCatalogue({ mode = 'products' }: { mode?: Mode }) {
  ...
  const reviewProduct = async () => {
    ...
    const { error: reviewError } = await supabase.rpc('admin_review_product', { p_admin_id: user.uid, p_product_id: approvalTarget.product.id, p_status: approvalTarget.status, p_admin_note: approvalTarget.note.trim() })
  ...
  return (
    <AdminShell>
      <AdminPageHeader title={title} description={mode === 'products' ? 'BikriKoro-এর live listing edit, publish ও archive করুন।' : mode === 'categories' ? 'প্রোডাক্ট taxonomy ও category name/order ম্যানেজ করুন।' : 'ভবিষ্যৎ industry taxonomy-এর জন্য app-native grouping।'} actions={mode === 'categories' ? <button onClick={addCategory} className="inline-flex items-center gap-2 rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-semibold text-white"><Plus size={16} />নতুন category</button> : undefined} />
