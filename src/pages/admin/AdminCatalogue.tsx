...
export default function AdminCatalogue({ mode = 'products' }: { mode?: Mode }) {
  ...
  const [moderationTarget, setModerationTarget] = useState<{ product: AdminProduct; action: 'HIDE' | 'RESTORE' } | null>(null)
  ...
  const moderate = async (product: AdminProduct, action: 'HIDE' | 'RESTORE') => {
    ...
    if (actionError) setError(formatAdminRpcError(actionError, 'প্রোডাক্ট visibility update', '050 admin product migration'))
    else await load()
  ...
  const deleteProduct = async (product: AdminProduct) => {
    ...
    setError(null)
    const { error: deleteError } = await adminRpc('admin_delete_product', { p_admin_id: user.uid, p_product_id: product.id })
    if (deleteError) {
      setError(formatAdminRpcError(deleteError, 'প্রোডাক্ট delete', '050 admin product migration'))
    } else {
