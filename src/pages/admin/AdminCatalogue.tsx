...
type ProductView = 'PENDING' | 'LIVE' | 'REJECTED' | 'DELETED'
...
export default function AdminCatalogue({ mode = 'products' }: { mode?: Mode }) {
  ...
  const filtered = useMemo(() => {
    ...
    const byView = products.filter((product) => {
      ...
      return Boolean(product.archived_at) || product.moderation_note === 'DELETE'
    })
  ...
  const viewCounts = useMemo(() => ({
    ...
    DELETED: products.filter((product) => Boolean(product.archived_at) || product.moderation_note === 'DELETE').length,
  }), [products])

  const toggleSelected = (id: string) => setSelectedIds((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id])
  const toggleAll = () => setSelectedIds((current) => current.length === filtered.length ? [] : filtered.map((product) => product.id))
