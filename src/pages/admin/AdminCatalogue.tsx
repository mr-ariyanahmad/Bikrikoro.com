...
export default function AdminCatalogue({ mode = 'products' }: { mode?: Mode }) {
  ...
  const moderate = async (product: AdminProduct, action: 'HIDE' | 'RESTORE' | 'DELETE') => {
    const message = action === 'RESTORE' ? 'এই listing আবার public করবেন?' : action === 'DELETE' ? 'এই listing archive করবেন? এটি public view থেকে সরবে।' : 'এই listing public view থেকে সরাবেন?'
  ...
  const addCategory = async () => {
    ...
    if (insertError) setError('ক্যাটাগরি যোগ করা যায়নি।')
    else load()
  }

  const renameCategory = async (category: Category) => {
    const name = window.prompt('Category-এর নতুন নাম', category.name)?.trim()
    if (!name || name === category.name) return
    const { error: updateError } = await supabase.rpc('admin_upsert_category', { p_admin_id: user?.uid, p_id: category.id, p_name: name, p_sort_order: category.sort_order })
    if (updateError) setError('ক্যাটাগরি আপডেট করা যায়নি।')
    else load()
  }
