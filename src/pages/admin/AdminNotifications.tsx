  ...
  return (
    <AdminShell>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
        <AdminTableCard>
          <div className="border-b border-slate-200 px-5 py-4"><div className="flex items-center gap-2"><Megaphone size={18} className="text-blue-600" /><h2 className="font-semibold text-slate-900">নতুন campaign</h2></div><p className="mt-1 text-xs text-slate-500">একবার পাঠালে in-app inbox-এ সংরক্ষিত হবে।</p></div>
...
function Status({ status }: { status: string }) {
  const tone = status === 'SENT' || status === 'IN_APP_SENT' ? 'bg-green-50 text-green-700' : status === 'PARTIAL' ? 'bg-amber-50 text-amber-700' : status === 'FAILED' ? 'bg-red-50 text-red-700' : 'bg-blue-50 text-blue-700'
