...
export default function AdminFinance() {
  ...
  const load = useCallback(() => {
    ...
    supabase.rpc('admin_list_withdrawals', { p_admin_id: user?.uid }).then(({ data, error: loadError }) => {
      ...
      if (loadError) setError('উইথড্রয়াল লোড করা যায়নি। নতুন admin migration প্রয়োগ করা হয়েছে কি না দেখুন।')
      setLoading(false)
    })
  }, [user?.uid])
  useEffect(() => { load() }, [load])
