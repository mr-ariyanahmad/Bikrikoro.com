...
export default function AdminContent({ mode }: { mode: Mode }) {
  ...
  return (
    <AdminShell>
      ...
      {loading ? <AdminTableCard><p className="p-10 text-center text-sm text-slate-500">লোড হচ্ছে...</p></AdminTableCard> : mode === 'gallery' ? <Gallery banners={banners} /> : mode === 'downloads' ? <Downloads downloads={downloads} /> : <Blog posts={posts} onStatus={updatePostStatus} showType={mode === 'pages'} />}
    </AdminShell>
  )
}
