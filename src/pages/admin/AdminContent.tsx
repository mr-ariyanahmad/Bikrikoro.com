...
export default function AdminContent({ mode }: { mode: Mode }) {
  ...
  const deleteBanner = async () => {
    ...
    const { error: deleteError } = await adminRpc('admin_delete_banner', { p_admin_id: user?.uid, p_banner_id: bannerToDelete.id })
  ...
  const updatePostStatus = async (post: Post, status: 'DRAFT' | 'PUBLISHED' | 'ARCHIVED') => {
    const { error: updateError } = await adminRpc('admin_set_content_status', { p_admin_id: user?.uid, p_id: post.id, p_status: status })
