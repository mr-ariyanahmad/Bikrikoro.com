...
import { Archive, Eye, Image as ImageIcon, Megaphone, Send, Upload } from 'lucide-react'
import { AdminPageHeader, AdminShell, AdminTableCard } from '@/components/admin/AdminShell'
import { useAuth } from '@/context/AuthContext'
import { formatDateTime } from '@/lib/format'
import { supabase } from '@/lib/supabase'

type Mode = 'gallery' | 'downloads' | 'blog'
...
type Post = { id: string; title: string; slug: string; excerpt: string; status: string; created_at: string }
...
export default function AdminContent({ mode }: { mode: Mode }) {
  ...
  const load = useCallback(() => {
    ...
    const request = mode === 'gallery'
      ...
      : mode === 'downloads'
        ...
        : supabase.rpc('admin_list_content', { p_admin_id: user?.uid, p_content_type: 'BLOG' })
  ...
  const savePost = async () => {
    ...
    const { error: saveError } = await supabase.rpc('admin_upsert_content', { p_admin_id: user?.uid, p_id: null, p_content_type: 'BLOG', p_title: postForm.title.trim(), p_slug: slug, p_excerpt: postForm.excerpt.trim(), p_body: postForm.body.trim(), p_status: 'DRAFT' })
    if (saveError) setError('Blog post সেভ করা যায়নি।')
  ...
  const title = mode === 'gallery' ? 'গ্যালারি' : mode === 'downloads' ? 'ডিজিটাল ডাউনলোড' : 'নিউজ ও ব্লগ'
  return (
    <AdminShell>
      <AdminPageHeader title={title} description={mode === 'gallery' ? 'Homepage banner ও promotional creative ম্যানেজ করুন।' : mode === 'downloads' ? 'Digital order delivery readiness দেখুন।' : 'BikriKoro-এর Bengali content draft, publish ও archive করুন।'} actions={mode !== 'downloads' && <button onClick={() => setShowForm((value) => !value)} className="inline-flex items-center gap-2 rounded-xl bg-[#0e6bdc] px-4 py-2.5 text-sm font-semibold text-white"><Upload size={16} />নতুন {mode === 'gallery' ? 'banner' : 'post'}</button>} />
      ...
      {showForm && mode === 'blog' && <AdminTableCard className="mb-5"><div className="space-y-3 p-5"><Field label="শিরোনাম" value={postForm.title} onChange={(value) => setPostForm({ ...postForm, title: value })} placeholder="আজকের marketplace update" /><Field label="সংক্ষিপ্ত বিবরণ" value={postForm.excerpt} onChange={(value) => setPostForm({ ...postForm, excerpt: value })} placeholder="কিছু কথায়..." /><label className="block text-sm text-slate-600"><span className="mb-1 block font-medium text-slate-800">মূল লেখা</span><textarea value={postForm.body} onChange={(e) => setPostForm({ ...postForm, body: e.target.value })} rows={6} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:border-blue-500" /></label><button onClick={savePost} className="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-semibold text-white">Draft সেভ করুন</button></div></AdminTableCard>}
      {loading ? <AdminTableCard><p className="p-10 text-center text-sm text-slate-500">লোড হচ্ছে...</p></AdminTableCard> : mode === 'gallery' ? <Gallery banners={banners} /> : mode === 'downloads' ? <Downloads downloads={downloads} /> : <Blog posts={posts} onStatus={updatePostStatus} />}
    </AdminShell>
  )
}

function Gallery({ banners }: { banners: Banner[] }) { return <AdminTableCard>{banners.length === 0 ? <div className="p-10 text-center"><ImageIcon className="mx-auto text-slate-300" size={32} /><p className="mt-3 text-sm text-slate-500">এখনো কোনো banner নেই।</p></div> : <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3">{banners.map((banner) => <div key={banner.id} className="overflow-hidden rounded-2xl border border-slate-200"><div className="aspect-[16/8] bg-slate-100">{banner.image_url && <img src={banner.image_url} alt="" className="h-full w-full object-cover" />}</div><div className="flex items-center justify-between p-3"><div><p className="text-xs text-slate-500">Sort {banner.sort_order}</p><p className="mt-1 truncate text-sm font-semibold text-slate-800">{banner.id}</p></div><Megaphone size={17} className="text-brand-600" /></div></div>)}</div>}</AdminTableCard> }
