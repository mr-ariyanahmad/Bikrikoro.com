...
type Thread = { id: string; buyer_id: string; seller_id: string; buyer_name?: string | null; seller_name?: string | null; product_id: string | null; last_message: string; last_message_at: string; buyer_unread_count: number; seller_unread_count: number }
export default function AdminSupport() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    adminRpc('admin_list_chat_threads', { p_admin_id: user?.uid }).then(async ({ data, error }) => {
      const base = (data ?? []) as Thread[]
      const ids = [...new Set(base.flatMap((thread) => [thread.buyer_id, thread.seller_id]).filter(Boolean))]
      const { data: profiles } = ids.length ? await supabase.from('profiles').select('id,name,email').in('id', ids) : { data: [] }
      const profileMap = new Map((profiles ?? []).map((profile) => [profile.id, profile]))
      setThreads(base.map((thread) => ({ ...thread, buyer_name: profileMap.get(thread.buyer_id)?.name || profileMap.get(thread.buyer_id)?.email, seller_name: profileMap.get(thread.seller_id)?.name || profileMap.get(thread.seller_id)?.email })))
