...
export default function AdminDisputes() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data, error: loadError } = await adminRpc('admin_list_pending_disputes')
    setDisputes((data ?? []) as OrderDispute[])
    if (loadError) setError(loadError.message || 'Dispute লোড করা যায়নি।')
    setLoading(false)
  }, [])
  useEffect(() => { load() }, [load])
