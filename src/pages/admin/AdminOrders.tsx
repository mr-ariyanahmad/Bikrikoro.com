...
export default function AdminOrders({ deliveriesOnly = false }: { deliveriesOnly?: boolean }) {
  ...
  return <AdminShell><AdminPageHeader title={deliveriesOnly ? 'ডেলিভারি' : 'অর্ডার ম্যানেজমেন্ট'} description={deliveriesOnly ? 'প্রতিটি delivery item card খুলে বিস্তারিত দেখুন।' : 'প্রতিটি order card খুললে payment, escrow, delivery, dispute ও payout state-এর পূর্ণ পেজ দেখা যাবে।'} actions={<Link to="/admin/orders" className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 hover:border-brand-400 hover:text-brand-700">রিফ্রেশ</Link>} />{error && <p className="mb-4 rounded-xl bg-red-50 p-4 text-sm text-red-700">{error}</p>}<div className="mb-5 grid gap-3 sm:grid-cols-[1fr_220px]"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="অর্ডার নম্বর, প্রোডাক্ট বা নাম খুঁজুন..." className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-brand-500" /><BrandSelect label="অর্ডারের status" value={status} options={statuses.map((value) => ({ value, label: value ? labels[value] : 'সব স্ট্যাটাস' }))} onChange={setStatus} /></div>{loading ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1, 2, 3, 4].map((item) => <div key={item} className="h-40 animate-pulse rounded-2xl bg-slate-100" />)}</div> : visible.length === 0 ? <AdminTableCard><p className="p-10 text-center text-sm text-slate-500">কোনো অর্ডার পাওয়া যায়নি।</p></AdminTableCard> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visible.map((order) => <Link key={order.id} to={`/admin/orders/${order.id}`} className="group rounded-2xl border border-outline bg-surface p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-md"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold text-slate-800">{order.product_title}</p><p className="mt-1 text-xs text-slate-400">{readableOrderNumber(order)}</p></div><span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${order.status === 'CANCELLED' ? 'bg-red-50 text-red-700' : order.status === 'COMPLETED' ? 'bg-brand-50 text-brand-700' : 'bg-amber-50 text-amber-700'}`}>{labels[order.status] ?? order.status}</span></div><div className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><p className="text-slate-400">ক্রেতা</p><p className="mt-1 truncate font-semibold text-slate-700">{order.buyer_name}</p></div><div><p className="text-slate-400">মোট</p><p className="mt-1 font-bold text-brand-700">{formatTaka(order.price + order.escrow_fee)}</p></div></div><div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3"><span className="text-xs text-slate-400">{formatDateTime(order.created_at)}</span><span className="text-xs font-bold text-brand-700 group-hover:translate-x-0.5">বিস্তারিত খুলুন →</span></div></Link>)}</div>}</AdminShell>
}
