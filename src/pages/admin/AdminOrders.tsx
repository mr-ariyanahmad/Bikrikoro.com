...
export default function AdminOrders({ deliveriesOnly = false }: { deliveriesOnly?: boolean }) {
  ...
  return (
    <AdminShell>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-[1fr_220px]"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="অর্ডার ID, প্রোডাক্ট বা user ID খুঁজুন..." className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-blue-500" /><select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-blue-500">{statuses.map((value) => <option key={value} value={value}>{value ? labels[value] : 'সব স্ট্যাটাস'}</option>)}</select></div>
      <AdminTableCard>
