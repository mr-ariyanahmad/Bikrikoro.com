...
export default function AdminSettings({ mode }: { mode: Mode }) {
  ...
  return <AdminShell><AdminPageHeader title={title} description={mode === 'invoice' ? 'Receipt ও invoice-এ যে business তথ্য দেখাবে তা সেট করুন।' : 'সাইটের brand, support এবং announcement তথ্য ম্যানেজ করুন।'} /><AdminTableCard><div className="space-y-4 p-5">{loading ? <div className="h-48 animate-pulse rounded-xl bg-slate-100" /> : <>{keys.map((key) => <label key={key} className="block text-sm text-slate-600"><span className="mb-1 block font-medium text-slate-800">{labelFor(key)}</span>{key.includes('address') || key.includes('announcement') ? <textarea value={values[key] ?? ''} onChange={(e) => setValues({ ...values, [key]: e.target.value })} rows={3} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:border-brand-500" /> : <input value={values[key] ?? ''} onChange={(e) => setValues({ ...values, [key]: e.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:border-brand-500" />}</label>)}<button type="button" onClick={save} disabled={saving} className="rounded-xl bg-brand-500 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{saving ? 'সেভ হচ্ছে...' : 'পরিবর্তন সেভ করুন'}</button></>}</div></AdminTableCard>{message && <p className={`mt-4 rounded-xl p-4 text-sm ${message.includes('হয়েছে') ? 'bg-brand-50 text-brand-700' : 'bg-red-50 text-red-700'}`}>{message}</p>}</AdminShell>
}
