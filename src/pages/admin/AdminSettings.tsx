...
export default function AdminSettings({ mode }: { mode: Mode }) {
  ...
  const keys = mode === 'invoice' ? ['invoice_prefix', 'invoice_business_name', 'invoice_phone', 'invoice_address'] : ['site_name', 'site_support_email', 'site_support_phone', 'public_support_email', 'public_support_phone', 'site_announcement', 'public_seo_title', 'public_seo_description', 'public_seo_og_image', 'public_seo_google_verification', 'public_payment_methods', 'public_trending_searches', 'reward_daily_checkin_coins', 'ai_help_enabled', 'ai_help_disclaimer']
  useEffect(() => {
    adminRpc('admin_get_settings', { p_admin_id: user?.uid, p_prefix: mode === 'invoice' ? 'invoice' : null }).then(({ data, error }) => {
