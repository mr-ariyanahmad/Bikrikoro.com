...
export function Layout({ children, wide = false, backFallback = '/', backLabel = 'ফিরে যান', hideFooter = false, fullScreen = false, hideMobileQuickNav = false }: { children: ReactNode; wide?: boolean; backFallback?: string; backLabel?: string; hideFooter?: boolean; fullScreen?: boolean; hideMobileQuickNav?: boolean }) {
  ...
  return (
    <div className={`site-minimal bg-bg text-ink-900 ${fullScreen ? 'flex h-[100dvh] min-h-0 flex-col overflow-hidden' : 'min-h-screen'}`}>
      <header className={`sticky top-0 z-40 border-b border-outline/35 bg-surface/95 shadow-[0_1px_8px_rgba(15,23,42,0.035)] backdrop-blur ${fullScreen ? 'shrink-0' : ''}`}>
        <div className={`mx-auto ${maxWidth} px-4 sm:px-5`}>
          <div className="flex min-h-[3.75rem] items-center gap-3 sm:gap-5">
            <Link to="/" className="flex shrink-0 items-center gap-2.5" onClick={closeMobileMenu}><img src="/icon-512.png" alt="BikriKoro" className="h-8 w-8 rounded-lg shadow-sm sm:h-9 sm:w-9" /><span className="hidden text-[15px] font-bold tracking-tight text-ink-900 sm:inline">BikriKoro<span className="text-brand-600">.Com</span></span></Link>
            ...
            <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
              ...
              <Link to="/notifications" className="relative rounded-xl p-2 text-ink-700 hover:bg-brand-50 hover:text-brand-700 md:hidden" aria-label="নোটিফিকেশন"><Bell size={20} />{user && unreadCount > 0 && <span className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-red-500 px-1 text-center text-[9px] font-bold leading-4 text-white">{unreadCount > 99 ? '99+' : unreadCount}</span>}</Link>
