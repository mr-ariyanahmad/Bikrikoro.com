...
export function Layout({ children, wide = false, backFallback = '/', backLabel = 'ফিরে যান', hideFooter = false, fullScreen = false, hideMobileQuickNav = false }: { children: ReactNode; wide?: boolean; backFallback?: string; backLabel?: string; hideFooter?: boolean; fullScreen?: boolean; hideMobileQuickNav?: boolean }) {
  ...
  return (
    <div className={`site-minimal bg-bg text-ink-900 ${fullScreen ? 'flex h-[100dvh] min-h-0 flex-col overflow-hidden' : 'min-h-screen'}`}>
      ...
      {!fullScreen && !hideMobileQuickNav && <nav aria-label="মোবাইল কুইক নেভিগেশন" className="fixed inset-x-0 bottom-0 z-50 rounded-t-[1.7rem] border-x border-t border-white/90 bg-surface/97 shadow-[0_-10px_30px_rgba(15,23,42,0.13)] backdrop-blur-xl md:hidden"><div className="mx-auto grid max-w-xl grid-cols-5 items-end px-2 pb-[max(env(safe-area-inset-bottom),0.7rem)] pt-2.5">{mobileQuickLinks.map((link) => <NavLink key={link.label} to={link.to} end={link.to === '/'} onClick={closeMobileMenu} className={({ isActive }) => `relative flex min-h-[3.65rem] min-w-0 flex-col items-center justify-end gap-1 rounded-2xl px-1 pb-1.5 text-[10px] font-extrabold tracking-[-0.01em] transition active:scale-[0.97] ${link.prominent ? isActive ? 'text-brand-800' : 'text-brand-700' : isActive ? 'bg-brand-50 text-brand-700 shadow-[inset_0_0_0_1px_rgba(1,124,80,0.12)]' : 'text-ink-500 hover:bg-bg hover:text-ink-800'}`}><span className={link.prominent ? ' -mt-8 flex h-14 w-14 items-center justify-center rounded-[1.2rem] border-4 border-surface bg-gradient-to-br from-brand-500 to-brand-700 text-white shadow-[0_10px_22px_rgba(1,124,80,0.28)] transition' : 'flex h-6 items-center justify-center'}><link.icon size={link.prominent ? 24 : 20} strokeWidth={link.prominent ? 2.35 : 1.9} /></span><span className="max-w-full truncate">{link.label}</span></NavLink>)}</div></nav>}
    </div>
  )
}
