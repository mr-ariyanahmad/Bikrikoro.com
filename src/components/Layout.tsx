...
export function Layout({ children, wide = false, backFallback = '/', backLabel = 'ফিরে যান', hideFooter = false, fullScreen = false, fullWidth = false, hideMobileQuickNav = false, hideMobileHeader = false }: { children: ReactNode; wide?: boolean; backFallback?: string; backLabel?: string; hideFooter?: boolean; fullScreen?: boolean; fullWidth?: boolean; hideMobileQuickNav?: boolean; hideMobileHeader?: boolean }) {
  ...
  return (
    <div className={`site-minimal bg-bg text-ink-900 ${fullScreen ? 'flex h-[100dvh] min-h-0 flex-col overflow-hidden' : 'min-h-screen'}`}>
      ...
      <main className={fullScreen ? `mx-auto ${maxWidth} flex min-h-0 w-full flex-1 flex-col overflow-hidden px-4 py-4 sm:px-5 sm:py-5` : `mx-auto ${maxWidth} px-4 ${isMarketplaceApp ? 'pb-8' : 'pb-[calc(7.5rem+env(safe-area-inset-bottom))]'} pt-5 sm:px-5 sm:pt-8 md:pb-8 md:py-8`}>{children}</main>
