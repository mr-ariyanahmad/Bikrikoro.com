...
import { ArrowRight, BadgeCheck, Clock3, Flame, Search, Sparkles, Tag, X } from 'lucide-react'
  ...
  return <div ref={containerRef} className={`relative ${compact ? 'w-full' : 'w-full max-w-md'}`}>
    <div className="relative">
      ...
      <input
        ...
        className={`w-full rounded-2xl border bg-white py-3.5 pl-11 pr-10 text-base text-ink-900 outline-none transition placeholder:text-ink-400 focus:border-brand-500 focus:bg-surface focus:ring-4 focus:ring-brand-500/10 ${compact ? 'border-brand-200 shadow-[0_6px_18px_rgba(1,124,80,0.10)]' : 'border-outline'}`}
        aria-expanded={!isFullSearchPage && open}
    ...
    {!isFullSearchPage && open && hasPanelContent && <div className={`absolute left-0 right-0 top-full z-50 mt-3 max-h-[min(40rem,calc(100vh-7rem))] overflow-y-auto rounded-3xl border border-brand-100 bg-surface shadow-[0_18px_55px_rgba(15,23,42,0.16)]`}>
      {showDiscovery && <>
        <section className="border-b border-brand-100 bg-gradient-to-br from-brand-50 via-white to-emerald-50/60 px-4 py-4 sm:px-5"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-brand-500 text-white shadow-sm"><Sparkles size={18} /></span><div><p className="text-sm font-extrabold text-ink-900">আপনি কী খুঁজছেন?</p><p className="mt-0.5 text-xs leading-5 text-ink-500">পণ্য, গেম, subscription বা কোনো shop-এর নাম লিখে খুঁজুন</p></div></div></section>
        {recentSearches.length > 0 && <section className="border-b border-outline p-3.5">
          ...
          <div className="flex flex-wrap gap-2">{recentSearches.map((term) => <button type="button" key={term} onClick={() => goToResults(term)} className="rounded-full border border-outline bg-white px-3 py-1.5 text-sm font-medium text-ink-700 shadow-sm transition hover:border-brand-400 hover:bg-brand-50 hover:text-brand-700">{term}</button>)}</div>
        </section>}
        {trendingSearches.length > 0 && <section className="border-b border-outline p-3.5">
          <div className="mb-2 flex items-center justify-between"><p className="flex items-center gap-2 text-sm font-bold text-ink-900"><Flame size={15} className="text-orange-500" />এখন বেশি সার্চ হচ্ছে</p><span className="text-[11px] font-medium text-ink-400">Top searches</span></div>
          <div className="flex flex-wrap gap-2">{trendingSearches.map((term) => <button type="button" key={term} onClick={() => goToResults(term)} className="rounded-full border border-brand-100 bg-brand-50 px-3 py-1.5 text-sm font-semibold text-brand-700 transition hover:border-brand-400 hover:bg-brand-100">{term}</button>)}</div>
        </section>}
        {categories.length > 0 && <section className="p-3.5">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{categories.slice(0, 9).map((category) => <button type="button" key={category.id} onClick={() => goToResults('', category.id)} className="flex min-h-11 items-center justify-between rounded-xl border border-outline bg-white px-3 py-2 text-left text-sm font-medium text-ink-700 shadow-sm transition hover:border-brand-400 hover:bg-brand-50 hover:text-brand-700"><span className="truncate">{category.name}</span><ArrowRight size={14} className="shrink-0 text-ink-300" /></button>)}</div>
        </section>}
      </>}
      ...
      {!showDiscovery && !loading && shops.length > 0 && <section aria-label="শপ ফলাফল" className="border-b border-outline p-3"><div className={compact ? 'grid grid-cols-2 gap-2.5' : 'space-y-2'}>{shops.map((shop) => <button type="button" key={shop.id} onClick={() => goToShop(shop)} className={`flex items-center gap-2.5 border border-outline bg-bg text-left transition hover:border-brand-300 hover:bg-brand-50 active:scale-[0.98] ${compact ? 'rounded-xl p-2.5' : 'w-full px-3 py-2.5'}`}><div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-100 font-bold text-brand-700">{shop.photo_url ? <img src={shop.photo_url} alt="" className="h-full w-full object-cover" /> : (shop.shop_name?.trim() || shop.name).charAt(0)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-ink-900">{shop.shop_name?.trim() || shop.name}</p><p className="mt-0.5 flex items-center gap-1 text-xs text-ink-500">{shop.is_verified && <BadgeCheck size={13} className="text-brand-600" />}ডিজিটাল শপ</p></div><ArrowRight size={15} className="shrink-0 text-ink-300" /></button>)}</div></section>}
      ...
      {query.trim() && <button type="button" onClick={() => goToResults()} className="flex w-full items-center justify-between border-t border-brand-100 bg-gradient-to-r from-brand-50 to-emerald-50 px-4 py-3.5 text-left text-sm font-bold text-brand-700 transition hover:from-brand-100 hover:to-emerald-100"><span>“{query.trim()}”-এর সব ফলাফল দেখুন</span><ArrowRight size={17} /></button>}
    </div>}
  </div>
}
