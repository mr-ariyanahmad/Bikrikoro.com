...
import { Banknote, Smartphone, X, ShieldCheck, WalletCards } from 'lucide-react'
...
export function BuyModal({ product, digitalSpecs, buyerId, onClose }: { product: Product; digitalSpecs?: ProductDigitalSpecs | null; buyerId: string; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 overflow-hidden bg-ink-900/55" role="dialog" aria-modal="true" aria-labelledby="confirm-order-title"><div className="flex h-full items-start justify-center sm:items-center sm:p-5"><div className="h-full w-full overflow-y-auto bg-surface p-4 sm:h-auto sm:max-h-[calc(100dvh-2.5rem)] sm:max-w-lg sm:border sm:border-outline sm:p-6 sm:shadow-2xl"><div className="flex items-center justify-between border-b border-outline pb-3"><div><p className="text-sm font-semibold text-brand-700">ডিজিটাল অর্ডার</p><h2 id="confirm-order-title" className="mt-1 text-lg font-semibold text-ink-900">অর্ডার নিশ্চিত করুন</h2></div><button type="button" onClick={onClose} className="p-1.5 text-ink-500 hover:bg-bg hover:text-ink-900" aria-label="বন্ধ করুন"><X size={19} /></button></div>
    <p className="mt-3 line-clamp-2 text-sm text-ink-700">{product.title}</p>
    <div className="mt-4 space-y-4"><div className="border border-brand-200 bg-brand-50 p-4"><div className="flex items-start gap-3"><ShieldCheck size={20} className="mt-0.5 shrink-0 text-brand-600" /><div><p className="text-sm font-semibold text-ink-900">সুরক্ষিত payment</p><p className="mt-1 text-xs leading-5 text-ink-700">পেমেন্ট UddoktaPay-এর নিরাপদ checkout-এ হবে। রিফান্ডের জন্য আপনার default account ব্যবহার করা হবে।</p></div></div></div>
      {!refundAccountReady && <Link to="/payment-accounts?purpose=BUYER_REFUND&required=1" onClick={onClose} className="flex items-start gap-3 border border-pink-200 bg-pink-50 p-4 text-pink-900"><WalletCards size={21} className="mt-0.5 shrink-0 text-pink-600" /><span><strong className="block">Refund account যোগ করা হয়নি</strong><span className="mt-1 block text-xs leading-5">প্রথম order-এর আগে একটি default refund account যোগ করুন।</span></span></Link>}
      ...
      <div className="rounded-2xl border border-outline bg-bg p-3"><p className="text-sm font-semibold text-ink-900">পেমেন্ট পদ্ধতি</p><div className="mt-3 flex items-center gap-3 rounded-xl border border-brand-500 bg-brand-50 p-3 text-brand-700"><Smartphone size={22} /><span className="flex-1"><span className="block text-sm font-bold">বাংলাদেশি online payment</span><span className="mt-1 block text-xs">বিকাশ · নগদ · রকেট · উপায় · UddoktaPay</span></span><span className="flex items-center gap-1.5"><img src="/payment-logos/bkash.png" alt="bKash" className="h-8 w-8 rounded-lg bg-white object-contain p-1 shadow-sm" /><img src="/payment-logos/nagad.png" alt="Nagad" className="h-8 w-8 rounded-lg bg-white object-contain p-1 shadow-sm" /><img src="/payment-logos/rocket.png" alt="Rocket" className="h-8 w-8 rounded-lg bg-white object-contain p-1 shadow-sm" /></span></div><p className="mt-3 flex items-center gap-2 text-[11px] leading-5 text-ink-500"><Banknote size={14} />Payment সম্পন্ন হলে পণ্যটি আপনার digital library-তে পাওয়া যাবে।</p></div>
      <div className="border border-outline p-3"><label className="mb-1.5 block text-sm font-medium text-ink-900" htmlFor="coupon-code">কুপন কোড</label><div className="flex gap-2"><input id="coupon-code" value={couponCode} onChange={(event) => { setCouponCode(event.target.value.toUpperCase()); setCoupon(null) }} placeholder="যেমন: WELCOME10" className="min-w-0 flex-1 border border-outline px-3 py-2 text-sm uppercase outline-none focus:border-brand-500" /><button type="button" onClick={() => void handleApplyCoupon()} disabled={couponLoading} className="border border-brand-500 px-3 py-2 text-sm font-semibold text-brand-600 disabled:opacity-50">{couponLoading ? 'যাচাই হচ্ছে…' : 'প্রয়োগ করুন'}</button></div>{coupon?.valid && <p className="mt-2 text-xs font-medium text-brand-700">কুপন প্রয়োগ হয়েছে: {coupon.message || 'ছাড় পাওয়া গেছে'}</p>}</div>
      <div className="bg-bg p-3 text-sm"><div className="flex justify-between text-ink-600"><span>পণ্যের দাম</span><span className="tabular-amount">{formatTaka(product.price)}</span></div>{coupon?.valid && <div className="mt-1 flex justify-between text-brand-700"><span>কুপন ছাড়</span><span className="tabular-amount">−{formatTaka(coupon.discount_amount)}</span></div>}<div className="mt-1 flex justify-between text-ink-600"><span>এসক্রো ফি</span><span className="tabular-amount">{formatTaka(escrowFee)}</span></div><div className="mt-2 flex justify-between border-t border-outline pt-2 font-semibold text-ink-900"><span>মোট</span><span className="tabular-amount">{formatTaka(total)}</span></div></div>
      <div className="flex items-start gap-2 border border-outline bg-bg p-3 text-xs leading-relaxed text-ink-600"><input id="order-policy-consent" type="checkbox" checked={acceptedPolicy} onChange={(event) => setAcceptedPolicy(event.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 border-outline text-brand-500 focus:ring-brand-500" /><label htmlFor="order-policy-consent">আমি BikriKoro-এর <Link to="/privacy" className="font-semibold text-brand-700 underline" onClick={(event) => event.stopPropagation()}>প্রাইভেসি পলিসি</Link> এবং <Link to="/return-policy" className="font-semibold text-brand-700 underline" onClick={(event) => event.stopPropagation()}>রিটার্ন ও রিফান্ড নীতি</Link> পড়েছি এবং মেনে নিচ্ছি।</label></div>
      {error && <p className="border border-error/30 bg-error/5 p-3 text-sm text-error">{error}</p>}
      <button type="button" onClick={() => void handleSubmit()} disabled={submitting || !acceptedPolicy || !refundAccountReady || !product.is_digital} className="w-full bg-brand-500 py-3 text-base font-semibold text-white hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50">{submitting ? 'পেমেন্ট সম্পন্ন হচ্ছে...' : 'UddoktaPay checkout-এ যান'}</button><p className="text-center text-xs text-ink-500">রিফান্ড হলে আপনার selected default account-এ পাঠানো হবে।</p>
    </div></div></div></div>
}
