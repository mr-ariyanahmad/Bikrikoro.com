...
drop policy if exists "Public products exclude moderated listings" on products;
...
drop policy if exists "Public reviews exclude moderated reviews" on reviews;
...
-- App-aware operational reads and mutations.
create or replace function admin_list_orders(p_admin_id text, p_status text default null)
...
create or replace function admin_list_customers(p_admin_id text)
...
create or replace function admin_list_withdrawals(p_admin_id text)
returns setof wallet_withdrawal_requests as $$
begin
    perform admin_assert(p_admin_id);
    return query select * from wallet_withdrawal_requests order by requested_at desc;
end;
$$ language plpgsql security definer;

create or replace function admin_review_withdrawal(p_admin_id text, p_request_id uuid, p_status text, p_admin_note text)
returns void as $$
begin
    perform admin_assert(p_admin_id);
    if p_status not in ('PENDING', 'APPROVED', 'REJECTED', 'PAID') then raise exception 'Invalid withdrawal status'; end if;
    update wallet_withdrawal_requests
    set status = p_status, admin_note = coalesce(p_admin_note, ''), processed_at = case when p_status in ('APPROVED', 'REJECTED', 'PAID') then now() else processed_at end
    where id = p_request_id;
    if not found then raise exception 'Withdrawal request not found'; end if;
end;
$$ language plpgsql security definer;

create or replace function admin_list_chat_threads(p_admin_id text)
returns setof chat_threads as $$
begin
    perform admin_assert(p_admin_id);
    return query select * from chat_threads order by last_message_at desc;
end;
$$ language plpgsql security definer;

create or replace function admin_send_notification(p_admin_id text, p_user_id text, p_title text, p_body text, p_link text default null)
returns uuid as $$
declare v_id uuid;
begin
    perform admin_assert(p_admin_id);
...
begin
    perform admin_assert(p_admin_id);
    if p_condition not in ('NEW', 'USED') or p_price <= 0 then raise exception 'Invalid product values'; end if;
    update products
    set title = trim(p_title), description = coalesce(p_description, ''), price = p_price,
        category_id = p_category_id, condition = p_condition, location = coalesce(p_location, ''), images = coalesce(p_images, images)
    where id = p_product_id;
    if not found then raise exception 'Product not found'; end if;
end;
$$ language plpgsql security definer;

create or replace function admin_set_content_status(p_admin_id text, p_id uuid, p_status text)
returns void as $$
begin
    perform admin_assert(p_admin_id);
    if p_status not in ('DRAFT', 'PUBLISHED', 'ARCHIVED') then raise exception 'Invalid content status'; end if;
    update admin_content set status = p_status, updated_at = now() where id = p_id;
    if not found then raise exception 'Content not found'; end if;
end;
$$ language plpgsql security definer;

create or replace function admin_list_digital_deliveries(p_admin_id text)
returns setof digital_deliveries as $$
begin
    perform admin_assert(p_admin_id);
    return query select * from digital_deliveries order by created_at desc;
end;
$$ language plpgsql security definer;
