-- Correct username normalization order: lowercase first, then remove unsupported characters.
do $$
declare
  rec record;
  v_base text;
...
begin
  for rec in select id, name, shop_name from public.profiles order by created_at loop
    v_base := regexp_replace(lower(coalesce(nullif(trim(rec.shop_name), ''), nullif(trim(rec.name), ''), 'shop')), '[^a-z0-9]+', '-', 'g');
