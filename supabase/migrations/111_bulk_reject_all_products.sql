-- BikriKoro — bulk Reject must apply to every selected product.
...
begin
  ...
  if p_action = 'APPROVE' then
    ...
    update public.products set approval_status = 'REJECTED', approval_note = coalesce(p_admin_note, ''), approval_reviewed_by = p_admin_id, approval_reviewed_at = now(), is_hidden = true where id = any(p_product_ids);
