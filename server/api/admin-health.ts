...
type SystemStatus = { profiles?: number; products?: number; orders?: number; pending_orders?: number; schema_marker?: string | null; updated_at?: string }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const [{ data: systemStatus, error: systemError }, { data: buckets, error: storageError }] = await Promise.all([
    ...
    const checks: HealthCheck[] = [
      ...
      check('schema_marker', 'Database schema marker', status.schema_marker ? 'OK' : 'MANUAL', status.schema_marker || 'চিহ্নিত নয়', status.schema_marker ? 'Repository-এর tracked schema marker পাওয়া গেছে।' : 'Schema marker migration এখনো সেট করা নেই।'),
