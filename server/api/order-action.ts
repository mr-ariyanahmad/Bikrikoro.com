...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === 'confirm_delivery' || action === 'buyer_confirm_digital') {
      if (!orderId) throw new Error('Order ID is required')
      const rpcName = action === 'buyer_confirm_digital' ? 'buyer_confirm_digital_delivery' : 'confirm_order_delivery'
      const result = await supabase.rpc(rpcName, { p_order_id: orderId, p_buyer_id: token.uid })
      if (result.error) throw result.error
    } else if (action === 'digital_deliver') {
      if (!orderId) throw new Error('Order ID is required')
      const result = await supabase.rpc('seller_deliver_digital', { p_order_id: orderId, p_seller_id: token.uid })
      if (result.error) throw result.error
      data = result.data
